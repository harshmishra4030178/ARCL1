...
const VisitorAnalyticsCard = ({ visitorData = null, loading = false, onRefresh = null }) => {
  ...
  return (
    <div className="bg-white rounded-2xl sm:rounded-3xl border border-gray-100 shadow-xs p-4 sm:p-6 md:p-8 space-y-5 sm:space-y-6">
      {/* HEADER */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 border-b border-gray-100 pb-4 sm:pb-5">
        ...
        {/* TAB CONTROLS */}
        <div className="w-full sm:w-auto flex items-center gap-1 bg-gray-50 p-1 rounded-xl sm:rounded-2xl border border-gray-200/70 overflow-x-auto text-xs font-semibold">
          <button
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-3.5">
        <div className="bg-slate-50/80 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-100 space-y-1">
          <div className="flex items-center justify-between text-[11px] sm:text-xs font-semibold text-gray-500">
            <span>Unique Visitors</span>
            <Users size={13} className="text-blue-600 sm:w-3.5 sm:h-3.5" />
          </div>
        ...
        <div className="bg-slate-50/80 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-100 space-y-1">
          <div className="flex items-center justify-between text-[11px] sm:text-xs font-semibold text-gray-500">
            ...
            <Eye size={13} className="text-indigo-600 sm:w-3.5 sm:h-3.5" />
          </div>
          <p className="text-xl sm:text-2xl font-black text-gray-900">{analytics.totalViews}</p>
          <span className="text-[10px] sm:text-[11px] text-gray-400">Storefront engagements</span>
        </div>

        <div className="bg-slate-50/80 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-100 space-y-1">
          <div className="flex items-center justify-between text-[11px] sm:text-xs font-semibold text-gray-500">
            <span>Desktop Share</span>
            <Monitor size={13} className="text-[#021C57] sm:w-3.5 sm:h-3.5" />
          </div>
        ...
        <div className="bg-slate-50/80 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-100 space-y-1">
          <div className="flex items-center justify-between text-[11px] sm:text-xs font-semibold text-gray-500">
            ...
            <Smartphone size={13} className="text-emerald-600 sm:w-3.5 sm:h-3.5" />
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6">
        ...
        <div className="lg:col-span-7 bg-slate-50/50 p-3.5 sm:p-5 rounded-2xl sm:rounded-3xl border border-gray-100 space-y-3 sm:space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div>
              <h3 className="text-xs sm:text-sm font-bold text-gray-800 flex items-center gap-1.5">
                <TrendingUp size={14} className="text-blue-600" /> Visitor Activity Timeline
              </h3>
            ...
            <div className="flex items-center gap-3 text-xs font-semibold">
              <div className="flex items-center gap-1.5">
                <span className="w-2 sm:w-2.5 h-2 sm:h-2.5 rounded-full bg-[#021C57]"></span>
                <span className="text-gray-600 text-[10px] sm:text-[11px]">Views</span>
              </div>
              <div className="flex items-center gap-1.5">
                <span className="w-2 sm:w-2.5 h-2 sm:h-2.5 rounded-full bg-emerald-500"></span>
                <span className="text-gray-600 text-[10px] sm:text-[11px]">Unique</span>
              </div>
            </div>
          </div>

          {/* Bar Chart Container */}
          <div className="relative pt-4 sm:pt-6 pb-2 overflow-x-auto">
            {hoveredDate && (
              <div className="absolute top-0 right-2 bg-gray-900 text-white px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-lg text-[10px] sm:text-xs z-10 flex items-center gap-2 animate-fadeIn shadow-md">
            ...
            <div className="h-36 sm:h-44 flex items-end justify-between gap-1 sm:gap-3 md:gap-4 border-b border-gray-200/80 px-1 sm:px-2 min-w-[280px]">
              {analytics.trend.map((item, idx) => {
                ...
                return (
                  <div
                    ...
                    className="flex-1 flex flex-col items-center gap-1 sm:gap-1.5 h-full justify-end group cursor-pointer"
                  >
                    <div className="w-full flex items-end justify-center gap-0.5 sm:gap-1 h-full">
                      {/* Pageviews Bar */}
                      <div
                        style={{ height: viewH }}
                        className={`w-2.5 sm:w-3.5 md:w-4 rounded-t-md transition-all duration-300 group-hover:brightness-110 ${
                          item.pageviews > 0
                    ...
                    <span className="text-[9px] sm:text-[10px] font-medium text-gray-500 group-hover:text-gray-900 transition whitespace-nowrap">
                      {item.date}
                    </span>
                  </div>
                );
        ...
        <div className="lg:col-span-5 bg-slate-50/50 p-3.5 sm:p-5 rounded-2xl sm:rounded-3xl border border-gray-100 space-y-4 flex flex-col justify-between">
          ...
          {activeTab === "devices" && (
            <div className="space-y-3.5 sm:space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-xs sm:text-sm font-bold text-gray-800 flex items-center gap-1.5">
                  <Monitor size={14} className="text-indigo-600" /> Device Distribution
                </h3>
              ...
              <div className="space-y-2.5 sm:space-y-3">
                ...
                <div className="bg-white p-2.5 sm:p-3 rounded-xl sm:rounded-2xl border border-gray-100 space-y-1 sm:space-y-1.5">
                  <div className="flex justify-between items-center text-xs">
                    <div className="flex items-center gap-1.5 sm:gap-2">
                      <Monitor size={13} className="text-[#021C57]" />
                ...
                <div className="bg-white p-2.5 sm:p-3 rounded-xl sm:rounded-2xl border border-gray-100 space-y-1 sm:space-y-1.5">
                  <div className="flex justify-between items-center text-xs">
                    <div className="flex items-center gap-1.5 sm:gap-2">
                      <Tablet size={13} className="text-amber-500" />
                ...
                {analytics.osBreakdown.length > 0 ? (
                  <div className="flex flex-wrap gap-1 sm:gap-1.5">
                    {analytics.osBreakdown.map((os, i) => (
                      ...
                      >
                        <Cpu size={10} className="text-gray-400" /> {os.name}:{" "}
          ...
          {activeTab === "browsers" && (
            <div className="space-y-3.5 sm:space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-xs sm:text-sm font-bold text-gray-800 flex items-center gap-1.5">
                  <Compass size={14} className="text-blue-600" /> Browser Market Share
                </h3>
          ...
          {activeTab === "pages" && (
            <div className="space-y-3.5 sm:space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-xs sm:text-sm font-bold text-gray-800 flex items-center gap-1.5">
                  <Layers size={14} className="text-emerald-600" /> Most Visited Storefront Routes
                </h3>
              ...
              {analytics.topPages.length > 0 ? (
                <div className="space-y-2">
                  {analytics.topPages.map((p, i) => (
                    ...
                    >
                      <span className="font-mono font-medium text-gray-700 truncate max-w-[140px] xs:max-w-[180px] sm:max-w-[240px] text-[11px] sm:text-xs">
                        {p.path}
                      </span>
