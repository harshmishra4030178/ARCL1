...
function Calibration() {
  ...
  const toggleVoiceSearch = () => {
    ...
    if (!SpeechRecognition) {
      alert("Voice search is supported in Google Chrome, Microsoft Edge, and Safari.");
      return;
    }
  ...
  const filteredCategories = useMemo(() => {
    ...
    return calibrationCategories
      ...
      .map((category) => {
        ...
        // 1. Direct and fuzzy match on items
        const filteredItems = category.items.filter((item) => {
          const itemLower = item.toLowerCase();
          return itemLower.includes(keyword) || fuzzyMatch(keyword, itemLower);
        });

        // 2. Direct and fuzzy match on category title & description
        const categoryMatches =
          category.title.toLowerCase().includes(keyword) ||
          category.description.toLowerCase().includes(keyword) ||
          fuzzyMatch(keyword, `${category.title} ${category.description}`);
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  placeholder={
                    isListening
                      ? "🎙️ Listening... Speak instrument name (e.g. 'Caliper', 'UTM', 'Pressure Gauge')..."
                      : "Search calibration instrument (e.g. Caliper, Force, UTM, Pressure, Thermometer, Micrometer)..."
                  }
                  className={`h-14 w-full rounded-2xl border ${
                    isListening
                      ? "border-red-500 ring-4 ring-red-400/20 bg-red-50/20"
                      : "border-slate-200 bg-white focus:border-cyan-400 focus:ring-4 focus:ring-cyan-100"
                  } pl-14 pr-24 text-sm font-medium outline-none transition shadow-xs`}
                />
              ...
              <button
                ...
                className="h-14 rounded-2xl border border-slate-200 bg-white px-6 text-sm font-bold text-slate-700 transition hover:border-slate-300 hover:bg-slate-100 cursor-pointer shrink-0"
              >
                Reset Filters
              </button>
            </div>

            {/* Live Search Status & Quick Filter Chips */}
            {search.trim() ? (
              <div className="flex items-center justify-between px-2 text-xs font-semibold text-slate-600">
                <span className="flex items-center gap-1.5 text-cyan-800">
                  ...
                  Found <strong>{totalMatchedCount}</strong> matching calibration instrument{totalMatchedCount !== 1 ? "s" : ""} across {filteredCategories.length} categor{filteredCategories.length !== 1 ? "ies" : "y"}
                </span>
                <button
                  onClick={() => setSearch("")}
              ...
              <div className="flex flex-wrap items-center gap-1.5 pt-1 px-1">
                ...
                {[
                  ...
                  <button
                    ...
                    className="text-xs bg-white hover:bg-cyan-50 hover:text-cyan-700 text-slate-600 px-2.5 py-1 rounded-lg border border-slate-200 transition font-medium cursor-pointer"
                  >
                    {item}
                  </button>
                ))}
